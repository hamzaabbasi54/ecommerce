      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center py-16 w-full">
          <div className="w-24 h-24 bg-[#f8f8f8] rounded-full flex items-center justify-center mb-6">
            <span className="material-symbols-outlined text-[40px] text-muted-foreground">favorite</span>
          </div>
          <h2 className="text-2xl font-bold text-foreground mb-3">Your wishlist is empty</h2>
          <p className="text-muted-foreground mb-8 text-center max-w-[400px] leading-relaxed">
            Save items you love to your wishlist. Review them anytime and easily move them to your cart.
          </p>
          <Link href="/products" className="px-8 py-3 bg-[#0063d1] text-white font-bold rounded-full hover:bg-[#004ca3] transition-colors inline-block">
            Browse Products
          </Link>
        </div>
      ) : (
        <div className="w-full border border-[#e5e5e5] rounded-sm mb-16">
          <Table className="text-center text-[13px]">
            <TableHeader className="bg-white">
              <TableRow className="border-b border-[#e5e5e5] hover:bg-transparent">
                <TableHead className="py-4 px-4 font-bold text-foreground text-center border-r border-[#e5e5e5] w-12"></TableHead>
                <TableHead className="py-4 px-4 font-bold text-foreground text-center border-r border-[#e5e5e5] w-48"></TableHead>
                <TableHead className="py-4 px-4 font-bold text-foreground text-center border-r border-[#e5e5e5]">Product name</TableHead>
                <TableHead className="py-4 px-4 font-bold text-foreground text-center border-r border-[#e5e5e5]">Unit price</TableHead>
                <TableHead className="py-4 px-4 font-bold text-foreground text-center border-r border-[#e5e5e5]">Stock status</TableHead>
                <TableHead className="py-4 px-4 font-bold text-foreground text-center"></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody className="bg-white">
              {items.map((item) => {
                  ...
                  return (
                    <TableRow key={item.id} className="border-b border-[#e5e5e5] last:border-b-0 hover:bg-transparent">
                      
                      {/* Remove Icon */}
                      <TableCell className="py-6 px-4 border-r border-[#e5e5e5] align-middle text-center">
                        <button 
                      ...
                      {/* Stock Status */}
                      <TableCell className="py-6 px-4 border-r border-[#e5e5e5] align-middle text-center text-muted-foreground">
                        {product?.stock > 0 ? 'In Stock' : 'Out of Stock'}
                      </TableCell>
                      
                      {/* Action */}
                      <TableCell className="py-6 px-4 align-middle text-center">
                        <button 
                          onClick={() => handleAddToCart(product.id)}
                          disabled={isAdding || product?.stock === 0}
                          className="text-[#0063d1] hover:underline disabled:opacity-50 disabled:no-underline mx-auto block"
                        >
