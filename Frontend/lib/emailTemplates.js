...
export const orderCancellationTemplate = (order, user) => {
    const itemsHtml = order.items.map(item => `
        <tr>
            <td style="padding: 12px 0; border-bottom: 1px solid #eaeaea;">
                <p style="margin: 0; font-size: 14px; font-weight: 500; color: #111;">${item.product ? item.product.name : 'Product'}</p>
    ...
    <head>
        ...
        <style>
            body { margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; background-color: #fafafa; color: #333; }
            .container { max-width: 600px; margin: 40px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #eaeaea; }
            .header { padding: 32px 40px; text-align: center; border-bottom: 1px solid #eaeaea; background-color: #fee2e2; }
            .header h1 { margin: 0; font-size: 24px; font-weight: 600; color: #b91c1c; letter-spacing: -0.5px; }
            .content { padding: 40px; }
            .greeting { margin-top: 0; font-size: 16px; color: #111; }
            .order-details { margin-top: 32px; }
            .order-details h2 { margin: 0 0 16px; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; color: #666; }
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <p class="greeting">Hi ${user.name},</p>
                <p style="font-size: 14px; color: #555; line-height: 1.5;">Per your request, we have successfully cancelled your order. If you paid by card, the refund has been initiated. Below are the details of the cancelled order.</p>
                
                <div class="order-details">
                    <h2>Cancelled Order #${order.id.slice(-8).toUpperCase()}</h2>
                    <table>
                        <tbody>
                            ${itemsHtml}
                        </tbody>
                    ...
                    <div class="totals">
                        ...
                        <div class="totals-row"><span>Shipping</span><span>$${order.shippingCharge.toFixed(2)}</span></div>
                        <div class="totals-row"><span>Tax</span><span>$${order.tax.toFixed(2)}</span></div>
                        <div class="totals-row bold"><span>Total Cancelled</span><span>$${order.total.toFixed(2)}</span></div>
                    </div>
                </div>
            </div>
            <div class="footer">
                <p>If you have any questions or did not request this cancellation, please reply to this email immediately.</p>
            </div>
        </div>
    </body>
    </html>
